...
<body>
	<pre>
	 ...
	 <!--<?php /* TODO: Material in Datenbank eintragen
	if(isset($_GET['daten'])) {
    $material = $_POST['material'];
	$mitarbeiter=array();
		
		print_r($material);
		while ($i <=sizeof($material)){
			$result = $db->query("
    	INSERT INTO material (materialbezeichnung, menge) values ('".$material[$i][materialbezeichnung]."','".$menge[$i][menge]."')
    ;");
		}
	}
		...
		*/?>-->
		...
		<div class="site-content">
			<form action="arbeitsbericht.php?daten=1" method="post">
				Logo <br/>
			...
			<button class="knopf"class="hinzubutton" >+</button> <br/> <br/>
				...
				<button class="knopf" class="knopfmaterialhinzu"  >+</button>
				</br> <br/>
				<table>
			
					<tr> <td>Pauschalen:</td>
						 <td><label><input type="Checkbox" name="Pauschalen[]" value="1" <?php if (in_array("1", $_POST['Pauschalen'])) echo "checked='checked'"; ?>/> Kleinmaterial</label></td>
						 <td><label><input type="Checkbox" name="Pauschalen[]" value="2" <?php if (in_array("2", $_POST['Pauschalen'])) echo "checked='checked'"; ?>/> Elektr. Kleinmaterial</label>
						 </td>
						<td><label><input type="Checkbox" name="Pauschalen[]" value="3" <?php if (in_array("3", $_POST['Pauschalen'])) echo "checked='checked'"; ?>/> Befestigungsmaterial</label></td>
						<td><label><input type="Checkbox" name="Pauschalen[]" value="4" <?php if (in_array("4", $_POST['Pauschalen'])) echo "checked='checked'"; ?>/> Isoliermaterial</label></td>
						<td><label><input type="Checkbox" name="Pauschalen[]" value="5" <?php if (in_array("5", $_POST['Pauschalen'])) echo "checked='checked'"; ?>/> Dichtmaterial</label> <br/></td>
					</tr>	
					<tr>
						<td></td>
						<td><label><input type="Checkbox" name="Pauschalen[]" value="6" <?php if (in_array("6", $_POST['Pauschalen'])) echo "checked='checked'"; ?>/> Lötmaterial</label></td>
						<td><label><input type="Checkbox" name="Pauschalen[]" value="7" <?php if (in_array("7", $_POST['Pauschalen'])) echo "checked='checked'"; ?>/> Vakuumpumpe</label></td>
						<td><label><input type="Checkbox" name="Pauschalen[]" value="8" <?php if (in_array("8", $_POST['Pauschalen'])) echo "checked='checked'"; ?>/> Lecksuchspray</label></td>
						<td><label><input type="Checkbox" name="Pauschalen[]" value="9" <?php if (in_array("9", $_POST['Pauschalen'])) echo "checked='checked'"; ?>/> D-Tec</label><br></td>
					</tr>
					<tr>
						<td>Arbeit beendet:</td>
						<td><label><input type="radio" name="beendet" value="Ja"<?php if (isset($_POST['beendet']) && $_POST['beendet'] == 'Ja') echo "checked='checked'"; ?>/> Ja</label></td>
						<td><label><input type="radio" name="beendet" value="Nein" <?php if (isset($_POST['beendet']) && $_POST['beendet'] == 'Nein') echo "checked='checked'"; ?>/> Nein</label></td>
						<td><label><input type="radio" name="beendet" value="Notdienst" <?php if (isset($_POST['beendet']) && $_POST['beendet'] == 'Notdienst') echo "checked='checked'"; ?>/> Notdienst</label> <br/></td>
					</tr>
					<tr>
						<td>Abrechnung:</td>
						<td><label><input type="radio" name="Abrechnung" value="pauschal" <?php if (isset($_POST['Abrechnung']) && $_POST['Abrechnung'] == 'pauschal') echo "checked='checked'"; ?>/> pauschal</label></td>
						<td><label><input type="radio" name="Abrechnung" value="zum Nachweis" <?php if (isset($_POST['Abrechnung']) && $_POST['Abrechnung'] == 'zum Nachweis') echo "checked='checked'"; ?>/> zum Nachweis</label></td>
						<td><label><input type="radio" name="Abrechnung" value="nach Aufmaß" <?php if (isset($_POST['Abrechnung']) && $_POST['Abrechnung'] == 'nach Aufmaß') echo "checked='checked'"; ?>/> nach Aufmaß</label><br/></td>
					</tr>
				</table>
				<br>
